...
async def main():
    ...
    def close_window():
        ...
        # For asyncio, stopping doesn't apply in the same way
        root.destroy()    

    # ... (Tkinter GUI setup code)
    ...
    send_button = tk.Button(column1_frame, text="Send Message", command=lambda: asyncio.create_task(send_message_async('192.168.200.22', 7000, 'Your_Message_Here')))
    send_button.grid(row=2, column=0)

    start_button = tk.Button(root, text="Start Receiving", command=lambda: asyncio.create_task(start_receiving_async('192.168.200.22', ["7000", "7010", "7020"], text_widgets)))
